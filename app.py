...
def home():  # put application's code here
    """Landing page for the platform root"""
...
def register():  # put application's code here
    ...
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor = mysql.connection.cursor()
        cursor.execute("INSERT INTO users (id, name, email, password) VALUES (%s, %s, %s, %s)", (str(uuid4()), name, email, hashed_password))
...
def categories():
    ...
    return render_template('categories.html', categories=pagination)
...
def units():
    ...
    return render_template('unit.html', units=pagination)
...
# @login_required(['admin', 'user'])
# def products():
#     """Disdplays all products with their relevant details"""
#     cursor = mysql.connection.cursor()
#     # categories dropdown
...
#     if search:
#         sql += " WHERE p.product_name LIKE %s OR c.name LIKE %s OR u.unit LIKE %s OR s.supplier_name LIKE %s ORDER BY p.created_at DESC"
...
#     cursor.close()
#     return render_template('products.html', categories=categories, units=units, suppliers=suppliers, products=products, search=search)
...
def products():
    ...
    return render_template('products.html', categories=categories, units=units, suppliers=suppliers, products=products, search_query=search_query, page=page, total_pages=total_pages)
...
def edit_products(id):
    ...
    if request.method == 'POST':
        ...
        cursor.close()
        return render_template('edit_products.html', product=product, categories=categories, units=units, suppliers=suppliers)
...
def analytics():
    ...
    cursor.execute("SELECT DATE_FORMAT(created_at, '%Y-%m-%d') AS date, COUNT(*) AS count FROM transactions GROUP BY DATE_FORMAT(created_at, '%Y-%m-%d')")
    ...
    return render_template('summary.html', product_count=product_count, category_count=category_count, unit_count=unit_count, supplier_count=supplier_count, labels=labels, counts=counts)
